#!/usr/bin/env python3
"""
Generate synthetic TAR dataset for benchmarking (no PIL required).

Creates WebDataset-style TAR files with dummy binary data.
"""
...
def generate_fake_jpeg(width=224, height=224):
    """Generate fake JPEG data (just random bytes)."""
    # Approximate size of a JPEG image
    size = width * height // 10  # ~10:1 compression
...
def generate_tar_shard(output_path, num_samples, image_size=(224, 224)):
    ...
    with tarfile.open(output_path, 'w') as tar:
        for i in range(num_samples):
            ...
            img_bytes = generate_fake_jpeg(*image_size)
...
def main():
    ...
    for f in os.listdir(args.output_dir):
        if f.endswith('.tar'):
            total_size += os.path.getsize(os.path.join(args.output_dir, f))

    print(f"Total size: {total_size / (1024**2):.1f} MB")
    print(f"Files created: {os.listdir(args.output_dir)}")
